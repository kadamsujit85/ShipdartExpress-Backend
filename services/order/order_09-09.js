                                                                            ...
                                                                            if (error) {
                                                                                ...
                                                                                mm.executeDML(`insert into transaction_master(CUSTOMER_ID, TRASACTION_TYPE, AMOUNT, EFFECT, TRANSACTION_DATETIME, CREATED_MODIFIED_DATE, ORDER_ID) values(?,?,?,?,?,?,?)`, [customerId, 'O', orderAmount, 'D', systemDate, systemDate, orderId], supportKey, connection, (error, insertTransaction) => {
                                                                                    if (error) {
                                                                                        ...
                                                                                        mm.executeDML(`select ID from delhivery_pickup_schedule where PICKUP_ADDRESS = ? AND date(PICKUP_DATETIME) = CURRENT_DATE AND CUSTOMER_ID = ? AND STATUS = 'P'`, [getShipNowList?.orderData?.[0]?.PICKUP_CONTACT_PERSON, customerId], supportKey, connection, (error, getScheduleData) => {
                                                                                            if (error) {
                                                                                                console.log(error);
                                                                                                mm.rollbackConnection(connection);
                                                                                                logger.error(req.url, req.method, JSON.stringify(error), req.baseUrl + req.url)
                                                                                ...
                                                                                mm.executeQueryData(query, [serviceId, DEAD_WEIGHT, volumetricWeight, DEAD_WEIGHT, volumetricWeight, carrierId, CUSTOMER_ID], supportKey, (error, getProductData) => {
                                                                                    if (error) {
                                                                                        ...
                                                                                        if (getProductData.length > 0) {
                                                                                            ...
                                                                                            if (getProductData[0].ADD_WEIGHT == 0) {
...
exports.createOrderForShopify = (req, res) => {
    ...
    jwt.verify(req.headers['token'], process.env.SECRET_KEY_FOR_SHOPIFY, (error, tokenData) => {
        if (error) {
            ...
            const payload = tokenData.data.PAYLOAD;
            const shopName = tokenData.data.SHOP_NAME;
            var systemDate = mm.getSystemDate()
                                                                                ...
                                                                                if (shipment_id) {
                                                                                    ...
                                                                                    request(options4, function (error, response, body4) {
                                                                                        if (error) {
                                                                                            ...
                                                                                            let codCommision = getOrder[0].PAYMENT_MODE == 'COD' ? (2.36 / 100) * orderAmount : 0;
