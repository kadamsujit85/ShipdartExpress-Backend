...
exports.shipnowOrderForDelhivery_old = async (req, res) => {
...
async function calculateUniversalRateForWeightDescripancy(supportKey, DEAD_WEIGHT, CUSTOMER_ID, PICKUP_PINCODE_ID, PINCODE_ID, PAYMENT_MODE, PRODUCT_ID, CARRIER_ID, SERVICE_ID, COD_AMOUNT, LENGTH, WIDTH, HEIGHT, ORDER_NO) {
    ...
    return new Promise((resolve, reject) => {
        try {
            console.log(DEAD_WEIGHT, CUSTOMER_ID, PICKUP_PINCODE_ID, PINCODE_ID, PAYMENT_MODE, PRODUCT_ID, CARRIER_ID, SERVICE_ID, COD_AMOUNT, LENGTH, WIDTH, HEIGHT, ORDER_NO);
...
exports.createOrderForShopify = (req, res) => {
    ...
    const shopifyHeaders = req.headers;
    var data = {
        "ORDER_DATETIME": systemDate,
        "MOBILE_NO": shopifyData.phone ? (shopifyData.phone).replace(/^(\+91)/, "") : '',
    ...
    try {
        if (orderDetails.length > 0) {
            ...
            mm.executeQueryData(`select CUSTOMER_ID from channel_customer_mapping where SHOP_NAME = ? AND STATUS = 1`, [shopifyHeaders['x-shopify-shop-domain']], supportKey, (error, getCustomerId) => {
                if (error) {
                    console.log(error)
                    logger.error(req.url, req.method, JSON.stringify(error), req.baseUrl + req.url)
                    res.send({
                                                                                        ...
                                                                                        if (error) {
                                                                                            ...
                                                                                            mm.executeDML(`update wallet_master set BALANCE = (BALANCE - ?) where CUSTOMER_ID = ?`, [orderAmount, customerId], supportKey, connection, (error, updateWallet) => {
                                                                                                if (error) {
                                                                                                    ...
                                                                                                    mm.executeDML(`update order_master set IS_SHIPPED = 1, SHIPPING_DATETIME = ?, AWB_NO = ?, CREATED_MODIFIED_DATE = ?, COURIER_API_DATA = ?, SERVICE_ID = ?, CARRIER_ID = ?, PRODUCT_ID = ?, ORDER_STATUS = 'S', ORDER_AMOUNT = ?,CHARGABLE_WEIGHT = ?, ORDER_STATUS_UPDATED_DATETIME = ? where ID = ? `, [systemDate, awbNo, systemDate, stringData, 0, 6, (serviceId == 1 ? 20 : 19), orderAmount, applicableWeight, systemDate, orderId,], supportKey, connection, (error, updateOrder) => {
                                                                                                        if (error) {
                                                                                                            console.log(error);
                                                                                                            mm.rollbackConnection(connection);
                                                                                                            logger.error(req.url, req.method, JSON.stringify(error), req.baseUrl + req.url)
